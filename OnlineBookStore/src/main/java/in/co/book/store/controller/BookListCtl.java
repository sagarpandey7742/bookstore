 ...
 */
@WebServlet(name = "BookListCtl", urlPatterns = { "/ctl/bookList" })
public class BookListCtl extends BaseCtl {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		long prId=DataUtility.getLong(request.getParameter("bookId"));
		if(prId>0) {
			BookBean pBean=new BookBean();
			pBean.setId(prId);
			try {
				model.delete(pBean);
